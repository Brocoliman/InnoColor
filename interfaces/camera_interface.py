...
class CameraInterface(Interface):
    def __init__(self, parent, controller):
        ...
        self.left_sidebar_width = 140
        self.right_sidebar_width = 180
        ...
        self.control_frame.pack(fill="both", expand=True, pady=60)  # Expands within the right sidebar

        ########## View Toggle Button (TOP) ##########
        self.tcx, self.tcy = 50, 50
        self.ta = 2

        self.toggle_button_canvas = Canvas(self.control_frame, width=100, height=100, bg="#000000", highlightthickness=0)
        self.toggle_button_canvas.pack(side="top", padx=self.right_sidebar_width/2, pady=20)  # Keep original padx
        ...
        self.capture_button_canvas = Canvas(self.control_frame, width=80, height=80, bg="#000000", highlightthickness=0)
        self.capture_button_canvas.pack(pady=10, padx=self.right_sidebar_width/2)  # Keep original padx

        # Draw iPhone-style button
        self.capture_button_canvas.create_oval(0, 0, 80, 80, fill="white", outline="gray", width=3)
        self.capture_button_canvas.create_oval(15, 15, 65, 65, fill="red", outline="")
        ...
        # Set default img
        thumbnail = Image.open('icons/icon_nav.png').resize((120, 120), Image.Resampling.LANCZOS)
        ...
        ########## Toolbar Frame ##########
        self.toolbar_container = Frame(self.left_sidebar, bg="#000000", pady=60)
        self.toolbar_container.pack(fill="both", expand=True, pady=20)

        ########## Model Button ##########
        self.mbutton = ModelButton(self.toolbar_container, self)
        self.mbutton.pack(anchor="center", padx=self.left_sidebar_width/2, pady=18) 
        ...
        self.sbutton.pack(anchor="center", padx=self.left_sidebar_width/2, pady=18)
        ...
        self.hbutton.pack(anchor="center", padx=self.left_sidebar_width/2, pady=18)

        ########## Initiate image pathway and camera feed ##########

        self.cam_feed.func = Process(self.mbutton, self.sbutton, self.hbutton)  # Set the function to process the image
