...
class ImageDataset_distiller(Dataset):
    def __init__(self, root, transforms_=None, mode="train", flip_flag=1):
        self.transform = transforms.Compose(transforms_)
        if mode == "None":
            self.files = sorted(glob.glob(root+ "/*.*"))
    ...
    def __getitem__(self, index):
        ...
        img = Image.open(self.files[index % len(self.files)]).convert("RGB")
        gt = Image.open(self.files[index % len(self.files)].replace('/Color_cvd_P_experiment_100000/','/gt_P_50/')).convert("RGB")
        if self.flip_flag==1:
            if np.random.random() < 0.5:
                img = Image.fromarray(np.array(img)[:, ::-1, :], "RGB")
