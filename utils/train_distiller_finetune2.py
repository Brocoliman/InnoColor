...
if CVD_type == 1:
    ...
    dis_index = 0
word_suffix = [
'_P_labonlyG+global_con+nature_points%d_ori_%s_ssimori_%s_ssim_%s_dis_dim_%s_finetune_nodis'%(opt.points,opt.ori,opt.ssimori,opt.lambda_ssim, opt.dim),
'_D_labonlyG+global_con+nature_points%d_ori_%s_ssimori_%s_ssim_%s_dis_dim_%s_finetune_nodis'%(opt.points,opt.ori,opt.ssimori,opt.lambda_ssim, opt.dim),
...
LUTs = torch.load("saved_models/cvd_100_001_D_labonlyG+global_con+nature_points3000_ori_1_ssimori_1_ssim_0.25_dis_dim_9/LUTs_100.pth")
LUT0.load_state_dict(LUTs["0"])
LUT1.load_state_dict(LUTs["1"])
LUT2.load_state_dict(LUTs["2"])
LUT3.load_state_dict(LUTs["3"])
LUT4.load_state_dict(LUTs["4"])
classifier.load_state_dict(torch.load("saved_models/cvd_100_001_D_labonlyG+global_con+nature_points3000_ori_1_ssimori_1_ssim_0.25_dis_dim_9/generator_100.pth"))
...
def generator_train(img):
    ...
    return combine_A.clip(0,1), pred
...
for epoch in range(opt.epoch+1, opt.n_epochs):
    ...
    for i, batch in enumerate(dataloader):
        ...
        loss_old = 0.0001 * tv_cons + 10 * mn_cons
        ...
        # Print log
        sys.stdout.write(
            "\r[Epoch %d/%d] [Batch %d/%d]  [G: %f,  local_c:%f, global_c:%f,ssim:%f,distiller:%f,tv:%f,mn:%f] ETA: %s"
            % (
                epoch,
                opt.n_epochs,
                i,
