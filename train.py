...
parser.add_argument("--dim", type=int, default=17, help="number of dim")
parser.add_argument(
...
if CVD_type == 1:
    ...
    dis_index = 0
word_suffix = [
'_P_%d_ori_%s_ssimori_%s_ssim_%s_dim17'%(opt.points,opt.ori,opt.ssimori,opt.lambda_ssim),
'_D_%d_ori_%s_ssimori_%s_ssim_%s_dim17'%(opt.points,opt.ori,opt.ssimori,opt.lambda_ssim),
...
def generator_train(img):
    ...
    LUT = generator(img_yuv).clip(0,1)
...
for epoch in range(opt.epoch+1, opt.n_epochs):
    ...
    for i, batch in enumerate(dataloader):
        ...
        # GAN loss
        fake_B = generator_train(real_A).clip(0,1)
