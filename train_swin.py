...
if CVD_type == 1:
    ...
    dis_index = 0
word_suffix = [
'_P_labonlyG+global_con+nature_points%d_ori_%s_ssimori_%s_ssim_%s_cvd50'%(opt.points,opt.ori,opt.ssimori,opt.lambda_ssim),
'_D_labonlyG+global_con+nature_points%d_ori_%s_ssimori_%s_ssim_%s_cvd50'%(opt.points,opt.ori,opt.ssimori,opt.lambda_ssim),
...
generator.load_state_dict(
        torch.load("saved_models_bak/cvd_100_001_P_labonlyG+global_con+nature_points3000_ori_1_ssimori_1_ssim_0.0/generator_30.pth"),strict=True)
...
def sample_images(batches_done):
    ...
    save_image(img_sample, "images/ssim/%s/%s.png" % (opt.dataset_name + word_suffix[dis_index], batches_done), nrow=5,
               normalize=True)
...
for epoch in range(opt.epoch+1, opt.n_epochs):
    ...
    for i, batch in enumerate(dataloader):
        ...
        if dis_index != -1 :
            ...
            # contrast loss
            cvd_fake = cvd_simulation_tensors(fake_B, CVD_type, 50)
            cvd_original = cvd_simulation_tensors(real_A, CVD_type, 50)
