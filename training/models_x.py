...
class Classifier(nn.Module):
    ...
    def forward(self, img_input):
        out = self.model(img_input)
        out = self.lr(out.view(img_input.shape[0], -1))
        return out
...
class Generator2DLUT_identity(nn.Module):
    def __init__(self, dim=33):
        ...
        self.LUT = nn.Parameter(torch.from_numpy(buffer).requires_grad_(False))

    def forward(self, x):
        x = (x - .5) * 2. # scale im between -1 and 1 since its used as grid input in grid_sample
        x = x.permute(0, 2, 3, 1)  # grid_sample expects NxDxHxWx3 (1x1xHxWx3)
        LUT = self.LUT[None].repeat(x.shape[0],1,1,1) # add batch dim to LUT
        result = F.grid_sample(LUT, x, mode='bilinear', padding_mode='border', align_corners=True)  # apply LUT
        result = result  # drop added dimensions
        return result
############################## TV & MN Calculations ##############################
# model = Generator2DLUT_identity(dim=17)
...
class TV_2D_LUT(nn.Module):  # only code difference is using LUT as value instead of object; for implementation, this is for fused LUT
    def __init__(self, dim=33):
        super(TV_2D_LUT,self).__init__()

        self.weight_u = torch.ones(2,dim,dim-1, dtype=torch.float)
        self.weight_u[:,:,(0,dim-2)] *= 2.0
        self.weight_v = torch.ones(2,dim-1,dim, dtype=torch.float)
        self.weight_v[:,(0,dim-2),:] *= 2.0
    ...
    def forward(self, LUT):
        ...
        dif_u = LUT[:,:,:-1] - LUT[:,:,1:]  # Red direction of LUT
        dif_v = LUT[:,:-1,:] - LUT[:,1:,:]  # Green direction of LUT
        
        tv = torch.mean(torch.mul((dif_u ** 2),self.weight_u)) + torch.mean(torch.mul((dif_v ** 2),self.weight_v))

        mn = torch.mean(self.relu(dif_u)) + torch.mean(self.relu(dif_v))
